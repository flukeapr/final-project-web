...
export async function PUT(req,{params}) {
    try {
        ...
        if (!allowedTypes.includes(image.type)) {
                return NextResponse.json({ message: "The file must be a .jpg or .png" }, { status: 400 });
            }
        ...
        await fs.writeFile(`./public/media/uploads/media-${isJpg?`${id}.jpg`:`${id}.png`}`, buffer);
        await query(`UPDATE media SET image = ? WHERE id = ?`, [`/media/uploads/media-${isJpg?`${id}.jpg`:`${id}.png`}`, id]);
